...
class COCOInstance(LightningDataModule):
    ...
    @staticmethod
    def polygons_to_mask(segmentation, width: int, height: int):
        ...
        **kwargs
        ...
        for label_id, cls_id in labels_by_id.items():
            ...
            if isinstance(segmentation, list):
                masks.append(
                    COCOInstance.polygons_to_mask(segmentation, width, height)
                )
            else:
    ...
    def _resolve_source_root(self):
        ...
        extracted_ready = (
            ...
            and (root / "val2017").is_dir()
            and (root / "annotations").is_dir()
        )

        if extracted_ready:
            return root, root

        return train_zip, annotations_zip

    def setup(self, stage: Union[str, None] = None) -> LightningDataModule:
        train_source, annotations_source = self._resolve_source_root()
        val_source = train_source if train_source.is_dir() else Path(self.path, "val2017.zip")
        dataset_kwargs = {
            "img_suffix": ".jpg",
        ...
        self.train_dataset = Dataset(
            ...
            target_zip_path=annotations_source,
            zip_path=train_source,
            **dataset_kwargs,
        )
