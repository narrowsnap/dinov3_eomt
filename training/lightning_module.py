...
def _silence_faster_coco_eval_logger():
...
class LightningModule(lightning.LightningModule):
    def __init__(
        ...
        if delta_weights and ckpt_path:
            ...
            ckpt = self._load_ckpt(ckpt_path, load_ckpt_class_head, current_state_dict)
    ...
    def init_metrics_instance(self, num_blocks):
        backend = (
            "faster_coco_eval"
            if importlib.util.find_spec("faster_coco_eval") is not None
            else "pycocotools"
        )
        if backend == "faster_coco_eval":
            _silence_faster_coco_eval_logger()
        self.metrics = nn.ModuleList(
            [
    ...
    def _on_eval_end_instance(self, log_prefix):
        if not self.trainer.sanity_checking:
            ...
            for class_idx in range(self.num_classes):
                metric_key = f"metrics/{log_prefix}_ap_class_{class_idx}"
                if metric_key not in self.trainer.callback_metrics:
                    continue
                class_ap_msgs.append(
                    f"class_{class_idx}: {self.trainer.callback_metrics[metric_key] * 100:.1f}"
                )
            if class_ap_msgs:
                rank_zero_info(
                    f"{bold_green}AP Per Class | " + " | ".join(class_ap_msgs) + reset
                )
            class_ap50_msgs = []
            for class_idx in range(self.num_classes):
                metric_key = f"metrics/{log_prefix}_ap_50_class_{class_idx}"
                if metric_key not in self.trainer.callback_metrics:
                    continue
                class_ap50_msgs.append(
                    f"class_{class_idx}: {self.trainer.callback_metrics[metric_key] * 100:.1f}"
                )
            if class_ap50_msgs:
                rank_zero_info(
                    f"{bold_green}AP50 Per Class | "
                    + " | ".join(class_ap50_msgs)
                    + reset
                )
            class_dice_msgs = []
            for class_idx in range(self.num_classes):
                metric_key = f"metrics/{log_prefix}_dice_class_{class_idx}"
                if metric_key not in self.trainer.callback_metrics:
                    continue
                class_dice_msgs.append(
                    f"class_{class_idx}: {self.trainer.callback_metrics[metric_key] * 100:.1f}"
                )
            if class_dice_msgs:
                rank_zero_info(
                    f"{bold_green}Dice Per Class | "
                    + " | ".join(class_dice_msgs)
                    + reset
                )
    ...
    def _adapt_ckpt_for_current_model(self, ckpt, current_state_dict):
        query_key = "network.q.weight"
        if query_key in ckpt and query_key in current_state_dict:
            ckpt_q = ckpt[query_key]
            current_q = current_state_dict[query_key]
            if (
                ckpt_q.ndim == 2
                and current_q.ndim == 2
                and ckpt_q.shape[1] == current_q.shape[1]
                and ckpt_q.shape[0] > current_q.shape[0]
            ):
                logging.info(
                    "Slicing %s from %s to %s",
                    query_key,
                    tuple(ckpt_q.shape),
                    tuple(current_q.shape),
                )
                ckpt[query_key] = ckpt_q[: current_q.shape[0]]
        return ckpt

    def _load_ckpt(self, ckpt_path, load_ckpt_class_head, current_state_dict=None):
        ...
        if current_state_dict is not None:
            ckpt = self._adapt_ckpt_for_current_model(ckpt, current_state_dict)
