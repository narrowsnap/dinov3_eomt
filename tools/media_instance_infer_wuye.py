...
DEFAULT_OOS_ANNOTATION_JSON = (
    "/mnt/FileExchange/users/zhouyang/instruments/data/五叶牵拉钳_box_dict_light.json"
)
...
def load_oos_annotation_map(path: str) -> dict[str, list[dict[str, Any]]]:
    ...
    if not json_path.is_file():
        print(f"特殊修正标注文件不存在，跳过规则: {json_path}")
        return {}
...
def infer_video_to_path(
    ...
    oos_label_name = "五叶牵拉钳"
    render_class_names = list(class_names)
    oos_label_id = (
        render_class_names.index(oos_label_name)
        if oos_label_name in render_class_names
        else len(render_class_names)
    )
            ...
            for frame_rgb, (masks, labels, scores) in zip(batch_rgb_list, outputs):
                ...
                if frame_index == 0 and video_oos_entries and not oos_locked:
                    for entry in video_oos_entries:
                        if str(entry.get("category_name", "")) != oos_label_name:
                            continue
                        bbox = entry.get("bbox")
                        ...
                        if matched_track_id is not None:
                            ...
                            oos_locked = True
                            print(
                                f"{oos_label_name} 命中: {input_video.name}, track_id={matched_track_id}, "
                                f"point=({center_x:.1f}, {center_y:.1f})"
                            )
                            break
                    if not oos_locked:
                        print(f"{oos_label_name} 未命中首帧预测实例: {input_video.name}")
                elif frame_index == 0 and not video_oos_entries:
                    print(f"{oos_label_name} 标注中未找到对应视频: {input_video.name}")
